    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            if (ip.Contains("C"))
            {
                ...
                notifyIcon1.BalloonTipText = "자비스는 네트워크 연결에 실패하였습니다";
            ...
            if (System.IO.File.Exists(path))
            {
                ...
                richTextBox1_DoubleClick(null, null);
            }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            /* if (!richTextBox1.Enabled&&!textBox1.UseSystemPasswordChar)
            {
                 textBox1.Enabled = false;
                         notifyIcon1.BalloonTipTitle = "ⓒ김주현 2015";
        ...
        private void richTextBox1_DoubleClick(object sender, EventArgs e)
        {
            ...
            //자비스에 필요한부분
            {
                foreach (string item in textValue)
                {
                    ...
                    if (s < 1 && item.Contains("자비스,") && item.Contains("강퇴"))
                    {
                        int which = item.IndexOf(">>");
                        if (item.Substring(which).Contains(ip))
                        {
                            string did1 = ("←" + ip + " 님이 " + DateTime.Now.ToString() + " 에 강퇴당하였습니다");
                            fs = new FileStream(path, FileMode.Append, FileAccess.Write);
                            sw = new StreamWriter(fs, System.Text.Encoding.Default);
                    ...
                    else if (item.Contains("자비스,") && item.Contains("메리크리스마스"))
                    {
                        ...
                        textBox1.Enabled = false;
                        notifyIcon1.BalloonTipTitle = "ⓒ김주현 @ 2015";
                        notifyIcon1.BalloonTipText = "Merry Christmas 명령에 따라\n당신을 인공지능이 강제퇴장시켰습니다";
                        notifyIcon1.ShowBalloonTip(3000);
                        Thread.Sleep(3000);
                        realexit = true;
                        this.Close();
